...
public class ListeEvenementController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getSession().getAttribute("sessionUser") == null)
            ...
            if (request.getParameter("button") != null) {
                ...
                else if (request.getParameter("button").equals("unregisterAction"))
                    this.unregisterAction(request,response);
            }
        }
    }
    ...
    /**
     * L'utiliser est supprimé de l'event
     * @param req request servlet
     * @param response response servlet
     * @throws IOException exception
     */
    private void unregisterAction(HttpServletRequest req, HttpServletResponse response) throws IOException {
        User user = userService.find(req.getSession().getAttribute("sessionUser"));
        if (req.getParameter("eventId") != null) {
            int idEvent = Integer.parseInt(req.getParameter("eventId"));
            Event event = evtService.find(idEvent);
